...
Mediator v1.0 (\verb|ipfixFields.xml|) a uká¾kový konfiguraèný súbor (\verb|config.xml|), ktorý je 
...
\subsubsection{In\v{s}tal\'acia samotnej aplik\'acie Mediator v1.0 pod OS Ubuntu/Debian}
...
je nastavi» správny port, na ktorom bude Mediátor oèakáva» IPFIX správy a parametre exportovacieho procesu.
...
Mediátor je konzolová aplikácia. Spustenie programu na operaèných systémoch Ubuntu/Debian pri in¹talácii 
...
Ïal¹ou podmienkou spustenia programu Mediator v1.0 je súbor \emph{ipfixFields.xml}. Cesta k~tomuto súboru sa 
...
% {|p{2cm}|p{1.7cm}|p{4cm}|p{4cm}|}
    ...
    \hline
    refreshTemplateTime & 5 & prirodzené celé èíslo väè¹ie ako 0 & èas (s) po ktorom Mediátor exportuje 
    príslu¹nú ¹ablónu (ak sa nastaví vy¹¹ia hodnota ako je v exportéri, tak reálne platí hodnota daná 
    exportérom)\\
    \hline
    exportTime & RENEW & KEEP, RENEW & spôsob, akým Mediátor narába s èasom v poli hlavièky správy - \uv{exportTime}\\
    \hline 
    \hline 
...
\end{verbatim}
\textbf{Popis a~rie¹enie:}
Port, na ktorom Mediátor prijíma pakety je obsadený iným programom, alebo iným Mediátorom. Treba skontrolova»
...
Po naimplementovaní vhodných sprostredkovateµských modulov mô¾e Mediátor vykonáva» rôzne predvýpoètové 
