...
Podporna trieda, ktora obsahuje pomocne metody potrebne pri de(kodovani) a pri validacii formatu dat. 
Uvadzam iba vlastne metody.
...
Konvertuje primitivny typ \emph{byte} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{byte} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
...
Konvertuje primitivny typ \emph{short} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{short} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
Pole bytov, typ \emph{byte[]}. \\
...
Konvertuje primitivny typ \emph{int} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{int} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
Pole bytov, typ \emph{byte[]}. \\
...
Konvertuje primitivny typ \emph{long} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{long} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
Pole bytov, typ \emph{byte[]}. \\
...
Konvertuje primitivny typ \emph{float} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{float} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
Pole bytov, typ \emph{byte[]}. \\
...
Konvertuje primitivny typ \emph{double} na pole bytov v usporadani bytov Big Endian. \\
\textbf{Parametre:} \\
\emph{double} x - hodnota, ktora sa ma zakodovat. \\
\textbf{Návratová hodnota:} \\
Pole bytov, typ \emph{byte[]}. \\
...
\textbf{Parametre:} \\
\emph{String} macAddress - adresa, ktorej format sa ma overit. \\
\textbf{Návratová hodnota:} \\
\emph{true} - v pripade, ze je adresa v spravnom formate.\\
\emph{false} - opacne.
...
Trieda so statickými metódami slú¾iacimi na zakódovanie retazcovej reprezentacie hodnot informacnych
elementov na abstraktne datove typy podla RFC 5101 a RFC 5102. Je presnym opakom triedy \verb|IPFIXDecoder|.
...
Zakóduje hodnotu datoveho typu do pola bytov podµa ¹pecifikácie IPFIX.
Priamo nevykonáva zakódovanie, volá konkrétne metódy podµa kategórie dátového typu.\\
{\bfseries Parametre:}\\
\emph{String} dataType - re»azec definujúci dátový typ obsiahnutý v~buffri.\\
...
\verb|OutOfBoundsException| - Ak je hodnota mimo povoleneho rozsahu\\
\verb|UnknownHostException| - Ak program nevie rozpoznat host, alebo IP adresu \\
\verb|DataException| - Ak je chyba v formate kodovanej hodnoty vzhladom na datovy typ\\
...
{\bfseries Hád¾e:} \\
\verb|UnsupportedDataException| - Ak dátový typ nie je podporovaný\\
\verb|NumberFormatException| - Ak je chyba v formate kodovanej hodnoty vzhladom na datovy typ\\
\verb|OutOfBoundsException| - Ak je hodnota mimo povoleneho rozsahu\\
...
dateTimeNanoseconds do pola bytov. \\
{\bfseries Parametre:}\\
\emph{String} dataType - re»azec definujúci dátový typ obsiahnutý v~buffri.\\
\emph{String} value - samotné dáta, ktoré sú predmetom zakódovania\\
{\bfseries Návratová hodnota:}\\
Pole bytov reprezentujúce interpretovanú hodnotu retazca na základe predaného typu.
...
Overi, ci ciselna hodnota v retazci spada do rozsahu daneho datovym typom. \\
{\bfseries Parametre:}\\
\emph{String} min - dolna hranica rozsahu \\
\emph{String} max - horna hranica rozsahu\\
\emph{String} value - hodnota, ktora sa ma overit\\
\emph{String} dataType - datovy typ hodnoty\\
{\bfseries Hád¾e:}\\
\verb|OutOfBoundsException| - Ak je hodnota mimo povoleneho rozsahu
...
\textbf{Parametre:}\\
\emph{IPFIXTemplateRecord} referencedTemplate - sablona patriaca datovym zaznamom\\
\emph{ArrayList$<$IPFIXDataRecord$>$} dataRecords - pole datovych zaznamov\\
\emph{IPFIXMessage.IPFIXMessageHeader} messageHeader - hlavicka IPFIX spravy, ktora obsahovala tento
zaznam o toku
...
\textbf{Parametre:}\\
\emph{IPFIXDataRecord} dataRecord - datovy zaznam, ktory sa priradi zaznamu o toku.



\subsubsection{Trieda IPFIXMessageHeader}
Trieda reprezentujuca hlavicku IPFIX spravy. Implementuje navrhovy vzor Singleton.
...
{\bfseries Návratová hodnota:}\\
Vracia jedinecnu instanciu objektu.





% ----------BALIK INTERMEDIATE PROCESS ----------------------------------
...
Abstraktna trieda, poskytuje vychodiskove metody sprostredkovatelskym procesom a tvori akesi
rozhranie medzi modulmi a aplikacnym ramcom. Dedi od triedy \verb|Thread|.\\
\\
\\
...
Nastavi meno procesu, podla prijateho parametra. Inicializuje vstupny buffer a ziska jedinecnu instanciu
...
Bezparametricky konstruktor. Inicializuje vstupny buffer a ziska jedinecnu instanciu
triedy \verb|IPFIXElements|.\\
...
\textbf{Parametre:}\\
\emph{Class} clazz = class objekt pozadovanej triedy \\
\textbf{Návratová hodnota:}\\
Objekt typu T, pricom T dedi od \verb|AIntermediateProcess|.
...
Java, teda nazov triedy vratane balickov, napr. 
\verb|sk.tuke.cnl.Mediator.SelectionProcess|.
Tato metóda naèíta \emph{class} objekt sprostredkovateµského procesu cez systémový class loader,
tak ako to bolo vy¹¹ie spomínané. Potom zavolá pôvodnú metódu \emph{getInstance(Class clazz)} a 
vráti in¹tanciu procesu.\\
\textbf{Parametre:}\\
\emph{String} processName = meno pozadovanej triedy \\
\textbf{Návratová hodnota:}\\
Objekt typu T, pricom T dedi od \verb|AIntermediateProcess|.\\
...
Pri prvom prechode funkciou sa generuje pamatovy zaznam o informacnych elementoch (ie) z XML suboru. 
Vytiahnu sa informacie o ie, ktore sa nachadzaju v sablone, dekoduju sa ich datove typy a prislusnost k 
skupine.\\
\textbf{Parametre:}\\
\emph{IPFIXTemplateRecord} template - sablona dat\\
\emph{IPFIXDataRecord} dataRecord - datovy zaznam\\
\textbf{Návratová hodnota:}\\
Dekodovane data ako objekt typu \verb|HashMap|.\\


\emph{protected final IPFIXDataRecord \textbf{encodeDataRecord}(IPFIXTemplateRecord template, 
HashMap$<$String, String$>$ dataMap)}

Zakoduje vsetky hodnoty z hashmapy obsahujucej hodnoty informacnych elementov podla sablony do datoveho 
zaznamu. \\
\textbf{Parametre:}\\
\emph{IPFIXTemplateRecord} template - sablona dat \\
\emph{HashMap$<$String, String$>$} dataMap - hodnoty informacnych elementov v hashmape, ktora sa ma zakodovat\\
\textbf{Návratová hodnota:}\\
Vracia objekt datoveho zaznamu -  \verb|IPFIXDataRecord|.\\
\textbf{Hád¾e:}\\
\verb|EncodingException| - Ak nastane chyba pri kodovani.\\
...
\textbf{Parametre:}\\
\emph{IPFIXFlowRecord} flowRecord - zaznam o toku, ktory sa ma posunut dalej \\
\emph{String} inputProcess - povodca zaznamu o toku 



\subsubsection{Trieda ExampleProcess}

Tato trieda je vzorovym riesenim jednoducheho sprostredkovatelskeho procesu, vykonavajuceho 
anonymizaciu. Ucelom triedy je pomoc dalsej generacii riesitelov.
...
Hlavna metoda vlakna. V cykle èaká na záznamy o tokoch vo svojom vstupnom bufferi \emph{(inputBuffer)} a 
...
Metoda na velmi jednoduchu anonymizaciu IP adresy, cislo v poslednom oktete zmeni na 0.\\
\textbf{Parametre:}\\
\emph{String} address - IP adresa, ktora sa anonymizovat \\
\textbf{Návratová hodnota:}\\
Anonymizovana IP adresa, vratena ako retazec.



\subsubsection{Trieda IPInputBuffer}

Reprezentuje vstupny buffer sprostredkovatelskych modulov. Tato trieda je cache pre zaznamy o tokoch. 
Jej pouzitie je kriticke vo vysokorychlostnych sietach, pretoze udrziava elementy a tym padom moze 
vyrovnavat narazovy napor. Je synchronizovana a jej implementacia je FIFO front typu \verb|ArrayBlockingQueue|.\\
...
\textbf{Parametre:}\\
\emph{IPFIXFlowRecord} flowRecord - zaznam o toku\\
\textbf{Návratová hodnota:}\\
...
Metoda obali prijate parametre do objektu tiredy \verb|IPFIXFlowRecord| a zavola predchadzajucu metodu.\\
\textbf{Parametre:}\\
\emph{IPFIXTemplateRecord} template - sablona\\
\emph{ArrayList$<$IPFIXDataRecord$>$} dataRecords - pole datovych zaznamov\\
\emph{IPFIXMessage.IPFIXMessageHeader} messageHeader - hlavicka IPFIX spravy, z ktorej tento zaznam o toku 
pochadza\\
...
\textbf{Hád¾e:}\\
\verb|InterruptedException| - Ak nastala chyba pri synchronizovani vlakien, alebo ak bolo vlakno prerusene 
pocas cakania.
...
Zjednodu¹ený diagram tried tohto balika mô¾eme vidie» na Obr. \ref{o:collecting1_class} a na 
...
\subsection{Balik sk.tuke.cnl.bm.Mediator.exporting}
...
Analogia k triede \verb|IPInputBuffer|. Reprezentuje exportovaciu cache, ktora je synchronizovana a 
jej implementacia je FIFO front typu \verb|ArrayBlockingQueue|.
...
Trieda sluzi na zakodovanie resp. zabalenie zaznamu o toku do IPFIX paketu podla specifikacie v RFC 5101 
a RFC 5102.\\
\\
...
\textbf{Návratová hodnota:}\\
Metoda vracia jedinecnu instanciu objektu triedy podla navrhoveho vzoru \emph{Singleton}.\\



\emph{protected byte[] \textbf{createIPFIXMessage}(IPFIXFlowRecord flowRecord)}

Na zaklade zaznamu o toku vytvara prud bytov z IPFIX spravy. Vola jednotlive motody, ktore robia 
ciastkove ulohy ako zakodovanie sad, hlavicky a podobne.\\
\textbf{Parametre:}\\
\emph{IPFIXFlowRecord} flowRecord - zaznam o toku\\
\textbf{Návratová hodnota:}\\
...
\textbf{Parametre:}\\
\emph{IPFIXMessage.IPFIXMessageHeader} header - hlavicka, ktora sa ma zakodovat\\
\emph{short} length - celkova dlzka IPFIX spravy\\
\textbf{Návratová hodnota:}\\
...
\textbf{Parametre:}\\
\emph{IPFIXTemplateRecord} templateRecord - zaznam sablony, ktory sa ma zakodovat\\
\textbf{Návratová hodnota:}\\
Pole bytov prudu zaznamu sablony.\\
...
\textbf{Parametre:}\\
\emph{byte[]} templateRecordBytes - pole bytov prudu zaznamov sablony, ktore obsahuje sada sablony\\
\textbf{Návratová hodnota:}\\
Pole bytov prudu sady sablon.\\
...
\textbf{Parametre:}\\
\emph{IPFIXDataRecord} dataRecord - datovy zaznam, ktory sa ma zakodovat\\
\textbf{Návratová hodnota:}\\
Pole bytov prudu datoveho zaznamu.\\
...
\textbf{Parametre:}\\
\emph{ByteArrayOutputStream} dataRecordsStream - prud bytov datovych zaznamov, ktore obsahuje sada sablony\\
\emph{int} templateID - ID prisluchajucej sablony \\
\textbf{Návratová hodnota:}\\
Pole bytov prudu datovej sady.
...
\textbf{Parametre:}\\
\emph{byte[]} templateSetBytes - pole bytov sady sablon\\
\emph{byte[]} dataSetBytes - pole bytov datovej sady\\
\emph{byte[]} optionsTemplateSetStream - pole bytov sady sablon moznosti\\
\textbf{Návratová hodnota:}\\
Celkova dlzka IPFIX spravy, vratane hlavicky. Navratovy typ je \emph{short}.
...
Trieda je samostatnym vlaknom, zaznamy o toku cita z ExportCache. Tie potom posle triede 
\verb|MessageEncoder|, ktora ich zabali do IPFIX sprav. Zakodovane spravy exportuje kolektoru na IP 
adresu a port, ktore su definovane v konfiguracnom subore. \\

\textbf{Kon¹truktor:}

\emph{private \textbf{UDPExporter}()}

Bezparametricky konstruktor. Vola rodicovsky konstruktor s parametrom svojho mena. Potom sa pokusi 
vytvorit socket pomocou triedy \verb|DatagramSocket|.\\
...
Hlavna metoda vlakna. Dokial nie je vlakno prerusene, cyklicky cita zaznamy o toku z ExportCache.
Prostrednictvom treidy \verb|MessageEncoder| vytvori zo zaznamu o toku vystupny prud bytov. 
Ten spolu s dlzkou prudu, IP adresou a portom zabali do paketu - objektu triedy \verb|DatagramPacket|.
Vzniknuty paket odosle.
